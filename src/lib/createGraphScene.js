...
export default function createGraphScene(canvas) {
  ...
  function loadGraph(newGraph, queryState) {
    if (scene) {
      scene.dispose();
      scene = null
      layoutSteps = 0;
    ...
    layout = createForceLayout(graph, queryState);
  ...
  function initUIElements() {
    ...
    graph.forEachNode(node => {
      ...
      node.ui = {size, position: [point.x, point.y, point.z || 0], color: 0x90f8fca0};
  ...
  function drawGraph() {
    ...
    graph.forEachNode(node => {
      ...
      let l = Math.hypot(body.force.x, body.force.y);
      if (l < minF) minF = l;
      if (l > maxF) maxF = l;
    });
    graph.forEachNode(node => {
      ...
      if (Math.abs(maxF - minF) > 1e-4) {
        let body = layout.getBody(node.id)
        let fx = body.force.x;
        let fy = body.force.y;
        let l = Math.floor(200 * (Math.sqrt(fx * fx + fy * fy) - minF)/(maxF - minF)) + 50;
        node.ui.color = (node.ui.color & 0xffffff00) | l;
      }
